...
namespace MovieShopMVC.Services
{
    public class CurrentLogedInUser : ICurrentLogedInUser
    {
        ...
        public string Email => _contextAccessor?.HttpContext?.User?.Claims.FirstOrDefault
            (c => c.Type == ClaimTypes.Email).Value;

        public string FullName => _contextAccessor.HttpContext.User.Claims.FirstOrDefault
